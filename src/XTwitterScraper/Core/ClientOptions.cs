...
public record struct ClientOptions()
{
    ...
    Lazy<string> _baseUrl = new(() =>
        Environment.GetEnvironmentVariable("X_TWITTER_SCRAPER_BASE_URL")
    ...
    /// <see cref="IXTwitterScraperClient.WithRawResponse"/>, don't perform validation until deserialization
    /// is triggered.</para>
    /// </summary>
    ...
    Lazy<string?> _apiKey = new(() =>
        Environment.GetEnvironmentVariable("X_TWITTER_SCRAPER_API_KEY")
    );
    ...
    Lazy<string?> _bearerToken = new(() =>
        Environment.GetEnvironmentVariable("X_TWITTER_SCRAPER_BEARER_TOKEN")
    );
